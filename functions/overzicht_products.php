...
$sqli_prepare = $con->prepare("SELECT p.productcode, p.naam, p.prijs, m.maat_naam, i.img_src, (SELECT GROUP_CONCAT(kleuren_lijst.kleur_naam) FROM product_kleur AS k JOIN kleuren_lijst on k.kleur_id = kleuren_lijst.id WHERE k.product_id = p.id) AS kleur_id FROM product_info AS p JOIN product_imgs AS i ON p.id = i.product_id JOIN maat_lijst as m ON p.maat = m.id  WHERE main_img = 1 AND categorie = ? ORDER BY naam");
